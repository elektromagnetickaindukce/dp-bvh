...
public:
    ...
    /// Second encoder for cylinders 
    MortonEncoder(const BoundingCyl<Scalar>& bbox, size_t grid_dim = max_grid_dim)
        : grid_dim(grid_dim)
    {
        assert(grid_dim <= max_grid_dim);
        world_to_grid = Scalar(grid_dim) * bbox.diagAABB().inverse();
        grid_offset = -bbox.getminpoint() * world_to_grid;
    }
